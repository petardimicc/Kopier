...
def copy_files(source_path, target_path):
    if not source_path or not target_path:
        messagebox.showerror("Error", "Source and target paths are required for Rsync.")
        return

    try:
        command = ["rsync", "-r", source_path, target_path]
        result = subprocess.run(command, capture_output=True, text=True)
        if result.returncode == 0:
            messagebox.showinfo("Success", "Files copied successfully using Rsync.")
        else:
            messagebox.showerror("Error", f"Failed to copy files using Rsync:\n{result.stderr}")
